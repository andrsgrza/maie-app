...
export class ImportModal extends BaseModal {
        ...
        try {
            ...
            if (!parsedDocument.metadata) {
                ...
                if (!parsedDocument.metadata.description) {
                    validationErrors.push("The 'metadata' object is missing the 'description' field.");
                }
                if (!parsedDocument.metadata.creationDate) {
                    validationErrors.push("The 'metadata' object is missing the 'creationDate' field.");
                }
            }
            ...
            if (!parsedDocument.sections) {
                ...
                parsedDocument.sections.forEach((section, index) => {
                    if (!section.title) {
                        validationErrors.push(`Section ${index + 1} is missing the 'title' field.`);
                    }
                    if (!section.items) {
                        validationErrors.push(`Section ${index + 1} is missing the 'items' field.`);
                    } else if (!Array.isArray(section.items)) {
                        validationErrors.push(`Section ${index + 1} should have an 'items' field that is an array.`);
                    } else {
                        section.items.forEach((item, itemIndex) => {
                            if (!item.question) {
                                validationErrors.push(`Item ${itemIndex + 1} in Section ${index + 1} is missing the 'question' field.`);
                            }
                            if (!item.answer) {
                                validationErrors.push(`Item ${itemIndex + 1} in Section ${index + 1} is missing the 'answer' field.`);
                            }
                        });
                    }
                });
    ...
    handleKeyDown = (event) => {
        if (event.key === 'Tab') {
            ...
            const updatedText = this.state.jsonText.substring(0, start) + '\t' + this.state.jsonText.substring(end);
                    ...
                    <ModalBody>
                        ...
                        <textarea
                            ...
                            style={{ 
                                ...
                                border: this.state.isValidJson ? '1px solid #ddd' : '1px solid red'
                            }}
